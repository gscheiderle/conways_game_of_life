...
<table>
  ...
  <tr>
    ...
    <tr>
    <td colspan="3">&nbsp;</td>
    <td> <br><button type="submit" name="raster_zeigen_3" value="raster_zeigen_3" style=" width: 250px; height: 40px; font-color: #000000; font-size: 22px; background-color: #00cc00; border-radius: 5px 5px 5px 5px;">Raster zeigen Tabelle 3</button> </td>
  </tr>
...
/* 
if ( $_POST['raster_zeigen_3'] == "raster_zeigen_3" ) {

include("spiel_ausgabe.php");
}
...
$select_gen=mysqli_query($link," select max(generation) as last_generation from table_game_3 ");
while ( $result_gen = mysqli_fetch_array($select_gen, MYSQLI_BOTH ) ) {
$last_generation=$result_gen['last_generation'];
$new_generation=$result_gen['last_generation']+1;
}
...
$where_klausel="where generation = '$last_generation' ";


$einlesen=mysqli_query($link," select id from table_game_3 $where_klausel "); 
while ( $einlesen_id = mysqli_fetch_array( $einlesen , MYSQLI_BOTH ) ) {

$einlesen_id=$einlesen_id['id'];


$color_istrue[]=$einlesen_id;
$id=$einlesen_id;
}
...
if(!is_dir("juni_generationen_im_bild"))
{mkdir("juni_generationen_im_bild");}

imagejpeg($gd, 'juni_generationen_im_bild/conway_'.$new_generation.'.jpg');
}  
